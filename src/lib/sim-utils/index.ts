 ...
 */
export function getSeededTimeInMinutes(
  seed: number,
  min: number,
  max: number,
): number {
...
export function convertMinutesToTimeString(minutes: number): string {
  if (typeof minutes !== "number" || minutes < 0 || minutes >= 24 * 60) {
    throw new Error(
      "Invalid input. Please provide a valid number representing time in minutes.",
    );
  }
 ...
 */
export function getRandomSqwuakCodeFromSeed(
  seed: number,
  airspaceId: string,
): string {
 ...
 */
export function getRandomFrequencyFromSeed(
  seed: number,
  name: string,
  objectId: string,
): Frequency {
  ...
  //   Fix the placeholder values - google what they should be
  return {
    value: `${prePointFrequency}.${afterPointFreq}`,
    unit: 4,
    type: 1,
    name: name,
    primary: true,
